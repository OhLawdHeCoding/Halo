  ...
  return (
    <button className="addButton" onClick={() => setItems(()=>{
        ...
        if (itemsIndex === -1){//check if the item exists in the itembox, if not we should add it and the new portion
            console.log("added");
            return [...items, {val:_val, portions:[{portion:_portion, amount:_portion.grams/1000*2}]}];
        }
        else {
            var portionIndex = items[itemsIndex].portions.findIndex(portionItem => portionItem.portion.grams == _portion.grams);
            if (portionIndex === -1){
                items[itemsIndex].portions.push({portion:_portion, amount:_portion.grams/1000*2});
                return [...items]
            }
            else{
            var oldAmount = items[itemsIndex].portions[portionIndex]; //items[val, [{PortionSize, ->COUNT<-}]]
            console.log("Pre inc: "+oldAmount);
            var newAmount = oldAmount+_portion.grams/1000; //tror denna koden kör två ggr per klick. Konstigt dock att console loggarna inte gör det
